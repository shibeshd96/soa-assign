...
@Path("beverages")
public class BeverageManagementResources {
	...
	public Response createCrate(final CratePostDTO newCrate, @Context final UriInfo uriInfo) {
		...
		return Response
		      .created(UriBuilder.fromUri(uriInfo.getBaseUri()).path(BeverageManagementResources.class).path(BeverageManagementResources.class, "getCrate").build(crate.getId())).build();
	}
